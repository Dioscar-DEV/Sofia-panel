...
-- =====================================================
-- PERMISOS PARA SERVICE_ROLE Y USUARIOS EN ESQUEMA INSTANCIAS
-- =====================================================
...
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA instancias TO authenticated, anon;
...
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA instancias TO authenticated, anon;
...
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA instancias TO authenticated, anon;
...
BEGIN
    ...
    -- Verificar si el usuario objetivo es superadmin
...
BEGIN
    ...
    -- Superadmin tiene acceso total por defecto
    IF user_role = 'superadmin' THEN
        RETURN TRUE;
    END IF;
...
CREATE POLICY "Users can edit own profile or users.edit" ON profiles
    ...
    USING (
        ...
        ((SELECT auth.uid()) = user_id)
        -- O tienen permiso users.edit O users.manage
    ...
    WITH CHECK (
        ...
        ((SELECT auth.uid()) = user_id)
        -- Si tiene permiso users.edit O users.manage, puede editar cualquier campo
        OR current_user_has_permission('users.edit')
        OR current_user_has_permission('users.manage')
    );
...
BEGIN
    -- Verificar si el usuario tiene permiso users.edit o users.manage
    ...
    -- Si el usuario está editando su propio perfil y NO tiene permisos de edición
    IF NEW.user_id = auth.uid() AND NOT (has_edit_permission OR has_manage_permission) THEN
        -- No puede cambiar el rol
        IF NEW.role != OLD.role THEN
            RAISE EXCEPTION 'No tienes permiso para cambiar tu rol. Se requiere permiso users.edit o users.manage';
        END IF;
        ...
        IF NEW.email != OLD.email THEN
            RAISE EXCEPTION 'No tienes permiso para cambiar tu email. Se requiere permiso users.edit o users.manage';
        END IF;
    END IF;
...
-- Asegurar columnas multimedia (idempotente)
ALTER TABLE instancias.agent_config
    ADD COLUMN IF NOT EXISTS multimedia_preprocessing BOOLEAN,
    ADD COLUMN IF NOT EXISTS multimedia_processing_config JSONB;

ALTER TABLE instancias.agent_config
    ALTER COLUMN multimedia_preprocessing SET DEFAULT false,
    ALTER COLUMN multimedia_processing_config SET DEFAULT '{"models": {"audio": "google/gemini-2.5-flash-lite", "image": "z-ai/glm-4.6v", "video": "z-ai/glm-4.6v", "document": "google/gemini-2.5-flash-lite"}, "detail_tier": 3}'::jsonb;

UPDATE instancias.agent_config
SET 
    multimedia_preprocessing = false,
    multimedia_processing_config = '{"models": {"audio": "google/gemini-2.5-flash-lite", "image": "z-ai/glm-4.6v", "video": "z-ai/glm-4.6v", "document": "google/gemini-2.5-flash-lite"}, "detail_tier": 3}'::jsonb
WHERE multimedia_preprocessing IS NULL;

ALTER TABLE instancias.agent_config
    ALTER COLUMN multimedia_preprocessing SET NOT NULL,
    ALTER COLUMN multimedia_processing_config SET NOT NULL;
...
CREATE TABLE IF NOT EXISTS instancias.agent_core_list (
    ...
    default_messaging_mode TEXT NOT NULL DEFAULT 'timeout_cancel'::text,
    ...
    CONSTRAINT agent_core_list_messaging_mode_check CHECK (default_messaging_mode IN ('timeout_cancel', 'timeout_aprove', 'aprove_all', 'cancel_all'))
);
...
-- Agregar columnas de messaging mode (idempotente)
ALTER TABLE instancias.agent_core_list
    ADD COLUMN IF NOT EXISTS default_messaging_mode TEXT,
    ADD COLUMN IF NOT EXISTS messaging_programming JSONB,
    ADD COLUMN IF NOT EXISTS timeout_ms NUMERIC;

-- Configurar defaults y constraints para messaging mode
ALTER TABLE instancias.agent_core_list
    ALTER COLUMN default_messaging_mode SET DEFAULT 'timeout_cancel'::text,
    ALTER COLUMN messaging_programming SET DEFAULT '[]'::jsonb,
    ALTER COLUMN timeout_ms SET DEFAULT '300000'::numeric;

UPDATE instancias.agent_core_list
SET 
    default_messaging_mode = 'timeout_cancel'::text,
    messaging_programming = '[]'::jsonb,
    timeout_ms = '300000'::numeric
WHERE default_messaging_mode IS NULL;

ALTER TABLE instancias.agent_core_list
    ALTER COLUMN default_messaging_mode SET NOT NULL,
    ALTER COLUMN messaging_programming SET NOT NULL,
    ALTER COLUMN timeout_ms SET NOT NULL;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'agent_core_list_messaging_mode_check'
    ) THEN
        ALTER TABLE instancias.agent_core_list ADD CONSTRAINT agent_core_list_messaging_mode_check CHECK (default_messaging_mode IN ('timeout_cancel', 'timeout_aprove', 'aprove_all', 'cancel_all'));
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS instancias.input_channels (
        ...
        CONSTRAINT input_channels_pkey PRIMARY KEY (name),
...
-- Asegurar columna core_supports en despliegues existentes (idempotente)
ALTER TABLE instancias.input_channels
    ADD COLUMN IF NOT EXISTS core_supports JSONB;

ALTER TABLE instancias.input_channels
    ALTER COLUMN core_supports SET DEFAULT '{"dm": false, "group": false, "support": false}'::jsonb;

UPDATE instancias.input_channels
SET core_supports = '{"dm": false, "group": false, "support": false}'::jsonb
WHERE core_supports IS NULL;

ALTER TABLE instancias.input_channels
    ALTER COLUMN core_supports SET NOT NULL;

-- Actualizar default de created_at (idempotente)
ALTER TABLE instancias.input_channels
    ALTER COLUMN created_at SET DEFAULT (now() AT TIME ZONE 'utc-4:00'::text);

-- Tabla de inputs de cores (configuración específica por canal)
CREATE TABLE IF NOT EXISTS instancias.cores_inputs (
    ...
    CONSTRAINT cores_inputs_custom_name_key UNIQUE (custom_name),
    CONSTRAINT cores_inputs_nameid_key UNIQUE (nameid),
    CONSTRAINT cores_inputs_canal_fkey FOREIGN KEY (canal) REFERENCES instancias.input_channels (name)
);

COMMENT ON TABLE instancias.cores_inputs IS 'Configuración específica de inputs para los cores del agente';

-- Tabla de inputs de instancias (configuración de canales activos)
CREATE TABLE IF NOT EXISTS instancias.instancias_inputs (
    ...
    output_options JSONB NOT NULL DEFAULT '{"text": true, "photo": false, "video": false, "gallery": false, "sticker": false, "document": false, "location": false}'::jsonb,
    meta_id TEXT NULL,
    status TEXT NOT NULL DEFAULT 'test'::text,
    CONSTRAINT instancias_inputs_custom_name_key UNIQUE (custom_name),
    CONSTRAINT instancias_inputs_meta_id_key UNIQUE (meta_id),
    CONSTRAINT instancias_inputs_nameid_key UNIQUE (nameid),
    CONSTRAINT instancias_inputs_canal_fkey FOREIGN KEY (canal) REFERENCES instancias.input_channels (name),
    CONSTRAINT instancias_inputs_status_check CHECK (status IN ('test', 'live', 'off'))
);

COMMENT ON TABLE instancias.instancias_inputs IS 'Configuración de inputs activos de las instancias del agente';

-- Asegurar columna meta_id (idempotente)
ALTER TABLE instancias.instancias_inputs
    ADD COLUMN IF NOT EXISTS meta_id TEXT;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'instancias_inputs_meta_id_key'
    ) THEN
        ALTER TABLE instancias.instancias_inputs ADD CONSTRAINT instancias_inputs_meta_id_key UNIQUE (meta_id);
    END IF;
END $$;

-- Asegurar columna status (idempotente)
ALTER TABLE instancias.instancias_inputs
    ADD COLUMN IF NOT EXISTS status TEXT;

ALTER TABLE instancias.instancias_inputs
    ALTER COLUMN status SET DEFAULT 'test'::text;

UPDATE instancias.instancias_inputs
SET status = 'test'::text
WHERE status IS NULL;

ALTER TABLE instancias.instancias_inputs
    ALTER COLUMN status SET NOT NULL;

-- Asegurar constraint de status (idempotente)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'instancias_inputs_status_check'
    ) THEN
        ALTER TABLE instancias.instancias_inputs ADD CONSTRAINT instancias_inputs_status_check CHECK (status IN ('test', 'live', 'off'));
    END IF;
END $$;

-- Tabla de pruebas de canales de entrada
CREATE TABLE IF NOT EXISTS instancias.input_channels_test (
    ...
    channel_nameid TEXT NOT NULL,
    CONSTRAINT input_channels_test_channel_nameid_fkey FOREIGN KEY (channel_nameid) REFERENCES instancias.instancias_inputs (nameid)
);

COMMENT ON TABLE instancias.input_channels_test IS 'Tabla para pruebas de conexión y flujo de datos en canales de entrada';
...
CREATE TABLE IF NOT EXISTS instancias.agent_contact_list (
        ...
        contact_docid TEXT,
        CONSTRAINT agent_contact_list_pkey PRIMARY KEY (user_id),
...
-- Migración: Corregir PK de agent_contact_list si es compuesta (idempotente)
DO $$
BEGIN
    -- Verificar si la PK actual incluye contact_system_channel (indicador de versión antigua)
    IF EXISTS (
        SELECT 1
        FROM information_schema.key_column_usage
        WHERE table_schema = 'instancias'
        AND table_name = 'agent_contact_list'
        AND constraint_name = 'agent_contact_list_pkey'
        AND column_name = 'contact_system_channel'
    ) THEN
        -- Eliminar la PK antigua
        ALTER TABLE instancias.agent_contact_list DROP CONSTRAINT agent_contact_list_pkey;
        -- Crear la nueva PK solo con user_id
        ALTER TABLE instancias.agent_contact_list ADD CONSTRAINT agent_contact_list_pkey PRIMARY KEY (user_id);
    END IF;
END $$;

-- Asegurar columna contact_chat NOT NULL (idempotente)
ALTER TABLE instancias.agent_contact_list
    ALTER COLUMN contact_chat SET NOT NULL;
...
-- Asegurar columna updated_at en despliegues existentes (idempotente)
ALTER TABLE instancias.agent_surveys
...
-- Asegurar columna updated_at en despliegues existentes (idempotente)
ALTER TABLE instancias.agent_task_list
    ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ;
...
DROP VIEW IF EXISTS instancias.v_tasks_summary;
...
-- Lista de callbacks para botones/acciones
...
COMMENT ON TABLE instancias.agent_callback_list IS 'Registro de endpoints para callbacks de botones y acciones';

-- Historial de ejecuciones de callbacks
...
COMMENT ON TABLE instancias.agent_callback_history IS 'Historial de ejecuciones de los callbacks';

-- Asegurar columna executed_at en despliegues existentes (idempotente)
ALTER TABLE instancias.agent_callback_history
    ADD COLUMN IF NOT EXISTS executed_at TIMESTAMPTZ;

ALTER TABLE instancias.agent_callback_history
    ALTER COLUMN executed_at SET DEFAULT (now() AT TIME ZONE 'utc-4:00'::text);

UPDATE instancias.agent_callback_history
SET executed_at = (now() AT TIME ZONE 'utc-4:00'::text)
...
-- Agregar columnas nuevas a agent_callback_history (idempotente)
ALTER TABLE instancias.agent_callback_history
    ADD COLUMN IF NOT EXISTS user_id TEXT REFERENCES instancias.agent_contact_list(user_id),
    ADD COLUMN IF NOT EXISTS channel_id TEXT REFERENCES instancias.instancias_inputs(nameid),
...
-- =====================================================
-- POLÍTICAS RLS PARA MÓDULOS DEL AGENTE (Frontend)
-- =====================================================

-- 1. Políticas para Contactos (agent_contact_list)
DROP POLICY IF EXISTS "Users with agent.contacts.view can view contacts" ON instancias.agent_contact_list;
CREATE POLICY "Users with agent.contacts.view can view contacts" ON instancias.agent_contact_list
    FOR SELECT TO authenticated
    USING (public.current_user_has_permission('agent.contacts.view'));

DROP POLICY IF EXISTS "Users with agent.contacts.manage can manage contacts" ON instancias.agent_contact_list;
CREATE POLICY "Users with agent.contacts.manage can manage contacts" ON instancias.agent_contact_list
    FOR ALL TO authenticated
    USING (public.current_user_has_permission('agent.contacts.manage'))
    WITH CHECK (public.current_user_has_permission('agent.contacts.manage'));

-- 2. Políticas para Tareas (agent_task_list)
DROP POLICY IF EXISTS "Users with agent.tasks.view can view tasks" ON instancias.agent_task_list;
CREATE POLICY "Users with agent.tasks.view can view tasks" ON instancias.agent_task_list
    FOR SELECT TO authenticated
    USING (public.current_user_has_permission('agent.tasks.view'));

DROP POLICY IF EXISTS "Users with agent.tasks.manage can manage tasks" ON instancias.agent_task_list;
CREATE POLICY "Users with agent.tasks.manage can manage tasks" ON instancias.agent_task_list
    FOR ALL TO authenticated
    USING (public.current_user_has_permission('agent.tasks.manage'))
    WITH CHECK (public.current_user_has_permission('agent.tasks.manage'));

-- 3. Políticas para Asignaciones de Tareas (agent_task_assign)
-- Heredan permisos de visualización de tareas, pero podrían requerir lógica específica si un usuario solo debe ver SUS asignaciones.
-- Por ahora, permitimos ver a quien tenga permiso de ver tareas globalmente (admin/superadmin).
DROP POLICY IF EXISTS "Users with agent.tasks.view can view assignments" ON instancias.agent_task_assign;
CREATE POLICY "Users with agent.tasks.view can view assignments" ON instancias.agent_task_assign
    FOR SELECT TO authenticated
    USING (public.current_user_has_permission('agent.tasks.view'));

DROP POLICY IF EXISTS "Users with agent.tasks.manage can manage assignments" ON instancias.agent_task_assign;
CREATE POLICY "Users with agent.tasks.manage can manage assignments" ON instancias.agent_task_assign
    FOR ALL TO authenticated
    USING (public.current_user_has_permission('agent.tasks.manage'))
    WITH CHECK (public.current_user_has_permission('agent.tasks.manage'));

-- 4. Políticas para Livechat (kpidata)
-- Conversaciones
DROP POLICY IF EXISTS "Users with agent.livechat.view can view conversations" ON kpidata.conversations;
CREATE POLICY "Users with agent.livechat.view can view conversations" ON kpidata.conversations
    FOR SELECT TO authenticated
    USING (public.current_user_has_permission('agent.livechat.view'));

-- Mensajes
DROP POLICY IF EXISTS "Users with agent.livechat.view can view messages" ON kpidata.messages;
CREATE POLICY "Users with agent.livechat.view can view messages" ON kpidata.messages
    FOR SELECT TO authenticated
    USING (public.current_user_has_permission('agent.livechat.view'));
...
CREATE TABLE IF NOT EXISTS kpidata.conversations (
    ...
    CONSTRAINT conversations_role_assign_fkey FOREIGN KEY (role_assign) REFERENCES roles (role_key),
    CONSTRAINT conversations_user_assign_fkey FOREIGN KEY (user_assign) REFERENCES profiles (user_id)
);

COMMENT ON TABLE kpidata.conversations IS 'Registro de conversaciones/chats del agente';

-- Multimedia Incoming (Reemplaza a multimedia antigua)
CREATE TABLE IF NOT EXISTS kpidata.multimedia_incoming (
    ...
    system_channel TEXT NOT NULL,
    prompt_id TEXT NULL,
    ext TEXT NULL,
    bucket_id TEXT NULL,
    CONSTRAINT multimedia_incoming_pkey PRIMARY KEY (media_id),
    CONSTRAINT multimedia_chat_id_fkey FOREIGN KEY (chat_id) REFERENCES kpidata.conversations (chat_id),
    CONSTRAINT multimedia_incoming_system_channel_fkey FOREIGN KEY (system_channel) REFERENCES instancias.input_channels (name),
    CONSTRAINT multimedia_user_channel_fkey FOREIGN KEY (user_channel) REFERENCES instancias.instancias_inputs (nameid),
    CONSTRAINT multimedia_user_id_fkey FOREIGN KEY (user_id) REFERENCES instancias.agent_contact_list (user_id)
);

COMMENT ON TABLE kpidata.multimedia_incoming IS 'Registro de multimedia entrante (ingesta)';
...
CREATE TABLE IF NOT EXISTS kpidata.messages (
    ...
    user_id TEXT NOT NULL,
    CONSTRAINT messages_chat_id_fkey FOREIGN KEY (chat_id) REFERENCES kpidata.conversations (chat_id),
    CONSTRAINT messages_media_id_fkey FOREIGN KEY (media_id) REFERENCES kpidata.multimedia_incoming (media_id),
    CONSTRAINT messages_user_id_fkey FOREIGN KEY (user_id) REFERENCES instancias.agent_contact_list (user_id)
);

COMMENT ON TABLE kpidata.messages IS 'Historial de mensajes de las conversaciones';
...
CREATE TABLE IF NOT EXISTS kpidata.multimedia_processing (
    ...
    CONSTRAINT multimedia_processing_media_id_fkey FOREIGN KEY (media_id) REFERENCES kpidata.multimedia_incoming (media_id)
);

COMMENT ON TABLE kpidata.multimedia_processing IS 'Detalles del procesamiento de multimedia por IA';

-- Mensajes internos (Legacy/Auditoría)
...
-- Eliminar tabla multimedia antigua si existe (Migración)
DROP TABLE IF EXISTS kpidata.multimedia CASCADE;

-- Permisos para service_role y usuarios en KPIDATA
...
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA kpidata TO authenticated, anon;

GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA kpidata TO service_role;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA kpidata TO authenticated, anon;

GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA kpidata TO service_role;
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA kpidata TO authenticated, anon;
...
-- =====================================================
-- SCHEMA MODULES - EXTENSIONES PERSONALIZADAS
-- =====================================================
-- Este esquema está reservado para tablas de módulos personalizados
-- que no son parte del núcleo (core) de la aplicación.

CREATE SCHEMA IF NOT EXISTS modules;

-- Permisos básicos para el esquema modules
GRANT USAGE ON SCHEMA modules TO service_role;
GRANT USAGE ON SCHEMA modules TO authenticated, anon;

-- Otorgar privilegios a service_role (backend/n8n)
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA modules TO service_role;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA modules TO service_role;
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA modules TO service_role;

-- Configurar defaults para futuras tablas en modules
...
-- NOTA: Los usuarios autenticados NO tienen acceso por defecto a las tablas de este esquema.
-- El acceso debe otorgarse explícitamente mediante políticas RLS (Row Level Security)
-- en cada tabla específica que se cree dentro de este esquema.
